...
# 양의 정수만 허용 — 음수/0/문자열은 기본 3으로 fallback
...
while [ "$COUNT" -lt "$MAX" ]; do
  ...
  if [ "$DRYRUN" = "1" ]; then
    if [ "$DRYRUN_FAIL" = "1" ]; then
      ...
      echo "run-queue: cycle $((COUNT+1)) aborted (DRYRUN_FAIL) — 즉시 종료"
      exit 0
    fi
    ...
    echo "run-queue: 실 trigger는 미구현 (Phase 3.1 schedule scope). AUTO_BUILD_QUEUE_DRYRUN=1로 검증하세요." >&2
    bash "$QUEUE_SH" status-update "$ID" "aborted" >/dev/null
    exit 1
  fi
